...
def _build_plain_text(date_key, articles, overview):
    lines = [f"Frontier Newsletter — {date_key}", f"{len(articles)} article{'s' if len(articles) != 1 else ''}", ""]
    if overview:
...
def send_newsletter(date_key, articles, overview, recipient, smtp_user=None, smtp_password=None,
    ...
    text_body = _build_plain_text(date_key, articles, overview)

    msg = MIMEMultipart("alternative")
    msg["Subject"] = f"Frontier Newsletter · {date_key} · {len(articles)} article{'s' if len(articles) != 1 else ''}"
    msg["From"] = smtp_user
    msg["To"] = recipient
